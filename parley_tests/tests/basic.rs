...
fn break_by_length_matches_max_advance_with_letter_spacing() {
    ...
    // Count clusters per line from the max_advance layout by summing cluster ranges of runs
    let cluster_counts: Vec<u32> = layout_max_advance
        .lines()
        .map(|line| {
            line.runs()
                .map(|run| run.cluster_range().len() as u32)
                .sum()
        })
        .collect();
    ...
    // Compare run cluster ranges
    for (i, (run_a, run_b)) in layout_max_advance
        .lines()
        .flat_map(|l| l.runs())
        .zip(layout_by_length.lines().flat_map(|l| l.runs()))
        .enumerate()
    {
        assert_eq!(
            run_a.cluster_range(),
